...
def randomizer():
    for i in KREWES: # goes through each key
        index = random.randrange(len(KREWES[i])) # determines a random number from 0 to the length of the key - 1
        print(KREWES[i][index]) # prints a random name from the key
